      ...
      async function apiFetch(path, options = {}) {
        ...
        try {
          ...
          data = { ok: false, error: "پاسخ نامعتبر سرور" };
        }
        return data;
      }
...
// ==================== DATA & STORAGE ====================
      ...
      function loadDB() {
        // همگام اولیه از local تا API برسد
        try {
      ...
      async function loadDBFromServer() {
        try {
          const res = await apiFetch("init.php");
          ...
          console.warn("PHP API offline, local mode", e);
        }
        return loadDB();
      }
      ...
      function saveDB(db) {
        ...
        if (window.__USE_PHP_API && db && (db.currentUser && (db.currentUser.role === "owner" || db.currentUser.role === "admin"))) {
          apiFetch("sync.php", { method: "POST", body: JSON.stringify(db) }).then((res) => {
            if (res && !res.ok) console.warn("sync failed", res.error);
          }).catch(() => {});
        }
      }
      ...
      function buildCategoryShelf(cat, products) {
        ...
        const cards = products
          .map((p) => {
            ...
            return (
              '<div class="product-card shelf-card reveal">' +
      ...
      function setupRevealAnimations(root) {
            ...
            (entries) => {
              entries.forEach((en) => {
                if (en.isIntersecting) {
                  ...
                  // آزاد کردن will-change بعد از انیمیشن
                  setTimeout(() => {
                    en.target.style.willChange = "auto";
                  }, 600);
                }
              });
            },
        ...
        scope.querySelectorAll(".reveal:not(.is-visible)").forEach((el, i) => {
          el.style.transitionDelay = Math.min(i * 0.04, 0.24) + "s";
          _revealObs.observe(el);
        });
      }

      // توقف انیمیشن وقتی تب مخفی است
      ...
      function cancelInlineEdits() {
        ...
        applySeasonTheme();
        render();
        setTimeout(function(){ setupRevealAnimations(document.getElementById('app')||document); }, 80);
      ...
      function renderHome() {
        ...
        <div class="trust-bar">
          <div class="container trust-bar-inner">
            <div class="trust-item"><span class="trust-ico">✓</span><span>محصولات ارگانیک</span></div>
            <div class="trust-item"><span class="trust-ico">✓</span><span>مشاوره تخصصی</span></div>
            <div class="trust-item"><span class="trust-ico">✓</span><span>ارسال سراسری</span></div>
            <div class="trust-item"><span class="trust-ico">✓</span><span>پشتیبانی واقعی</span></div>
          </div>
        </div>
        <section class="section section-cats">
          <div class="container">
            <div class="section-header">
              ...
              <p>انتخاب سریع بر اساس نیاز کشت و باغ</p>
            </div>
            <div class="cat-tiles" id="homeCatTiles"></div>
          </div>
        </section>
        ...
        const tiles = document.getElementById("homeCatTiles");
        if (tiles) {
          const cats = getAllCategories();
          tiles.innerHTML = cats
            .map((cat) => {
              const n = productsInCategory(cat, true).length;
              return (
                '<button type="button" class="cat-tile reveal" onclick="navigate(\'products\')">' +
                '<span class="cat-tile-ico">▣</span>' +
                "<strong>" +
                escapeHtml(cat) +
                "</strong>" +
                "<small>" +
        ...
        if (shelves) {
          ...
          setTimeout(function(){ setupRevealAnimations(document.getElementById('app')); }, 50);
        }
      }
      ...
      window.filterProductShelves = function () {
        ...
        setTimeout(function(){ setupRevealAnimations(document.getElementById("app")); }, 30);
      };
      ...
      window.sendLoginOtp = async function () {
        ...
        if (!mobile || mobile.length < 10) {
          showToast("شماره موبایل معتبر وارد کنید (مثال: 09120000000)", "error");
          return;
        }
        try {
          ...
          if (!res || !res.ok) {
            showToast((res && res.error) || "ارسال کد ناموفق بود", "error");
            return;
          }
          window._loginOtp = { mobile, expires: Date.now() + 5 * 60 * 1000 };
          const hint = document.getElementById("otpHint");
          if (hint) {
            const codeShow = res.dev_code
              ? ' <span style="color:var(--primary)">کد تست: <b dir="ltr">' + res.dev_code + "</b></span>"
              : "";
            hint.innerHTML =
              'کد برای <b dir="ltr">' + mobile + "</b> درخواست شد." + codeShow +
              " همین کد را در کادر کد تأیید بنویسید.";
          }
          if (res.dev_code) {
            const otpInput = document.getElementById("loginOtp");
            if (otpInput) otpInput.value = res.dev_code;
          }
          showToast("کد تأیید آماده است — آن را وارد کنید");
          const otpInput = document.getElementById("loginOtp");
          ...
          showToast("ارتباط با سرور برقرار نشد. با http://localhost باز کنید", "error");
        }
      };
      ...
      window.handleLogin = async function () {
        ...
        if (!otpEntered) {
          showToast("ابتدا «ارسال کد» را بزنید و کد را وارد کنید", "error");
          return;
        }
        if (!email || !password) {
        ...
        try {
          ...
          if (!res || !res.ok) {
            showToast((res && res.error) || "ورود ناموفق بود", "error");
            return;
          }
          const user = res.user;
          DB.currentUser = {
          ...
          // همگام‌سازی دوباره از سرور
          try {
            const full = await loadDBFromServer();
            if (full) {
              DB = full;
              DB.currentUser = {
                id: user.id,
          ...
          if (user.role === "admin" || user.role === "owner") {
            showQuickAdminBar();
            try { syncQuickSeasonSelect(); } catch (e) {}
          } else {
            hideQuickAdminBar();
          }
          render();
        } catch (e) {
          showToast("خطا در ارتباط با سرور PHP — سایت را با localhost باز کنید", "error");
          console.error(e);
        }
      };
        ...
        <div class="admin-layout">
            ...
            <aside class="admin-sidebar" id="adminSidebar">
                ...
                <nav class="admin-nav">
                    ...
                    ${link("settings", "#admin/settings", "⚙ تنظیمات", "navigate('admin','settings')")}
            ...
            <div class="admin-folder-grid" id="adminFolderGrid">
              ${cats
                ...
                .map((c) => {
                  ...
                  return `
                  <div class="admin-folder-card-wrap">
                    ...
                    <button type="button" class="btn btn-sm admin-folder-del" onclick="deleteCategory('${String(c).replace(/'/g, "\'")}', event)">حذف</button>
                  </div>`;
                })
      ...
      window.deleteCategory = function (name, ev) {
        if (ev) {
          ev.preventDefault();
          ev.stopPropagation();
        }
        if (!name) return;
        const count = productsInCategory(name, false).length;
        const msg =
          count > 0
            ? "دسته «" + name + "» دارای " + count + " محصول است. با حذف دسته، محصولات بدون دسته می‌شوند. ادامه؟"
            : "دسته «" + name + "» حذف شود؟";
        if (!confirm(msg)) return;
        DB.categories = (DB.categories || []).filter((c) => c !== name);
        DB.products.forEach((p) => {
          if (p.category === name) p.category = "عمومی";
        });
        if (!DB.categories.includes("عمومی") && DB.products.some((p) => p.category === "عمومی")) {
          DB.categories.push("عمومی");
        }
        saveDB(DB);
        showToast("دسته‌بندی حذف شد");
        logActivity("حذف دسته‌بندی", name);
        const el = document.getElementById("adminContent");
        if (el) renderAdminProducts(el);
      };
...
window.updateTicketStatus = function (id, status) {
      ...
      function renderAdminPages(el) {
        el.innerHTML = `
          <div class="admin-header">
            ...
            <button class="btn btn-primary" onclick="openPagesEditor()">✎ ویرایش همه صفحات</button>
          </div>
      ...
      // Bootstrap from PHP brain
      (async function bootstrapPhpBrain() {
        try {
          const remote = await loadDBFromServer();
          if (remote) {
            DB = remote;
